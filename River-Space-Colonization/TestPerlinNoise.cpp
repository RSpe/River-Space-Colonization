...
namespace test
{
	...
	float TestPerlinNoise::PerlinResult(float x, float y, float z)
	{
		float total = 0;
		float frequency = 0;
	...
	float TestPerlinNoise::Generation(float x, float y, float z)
	{
		/* Left bound */
		int x0 = (int)x & 255;
		int y0 = (int)y & 255;
	...
	void TestPerlinNoise::SetAmplitude(int amplitude)
	{
		amp = amplitude;
	}

	void TestPerlinNoise::SetOctave(int octave)
	{
		oct = octave;
	}

	void TestPerlinNoise::SetPersistence(int persistence)
	{
		persist = persistence;
	}

	float TestPerlinNoise::Gradient(int hash, float x, float y, float z)
	{
		int h = hash & 15;
	...
	float TestPerlinNoise::Lerp(float a, float b, float x)
	{
		return ((1.0f - x) * a) + (x * b);
	}

	float TestPerlinNoise::Fade(float t)
	{
		/* Smoothing function */
		return t * t * t * (t * (t * 6 - 15) + 10);
	}

	void TestPerlinNoise::OnUpdate(float deltaTime)
	{

	}

	void TestPerlinNoise::OnRender()
	{

	}

	void TestPerlinNoise::OnImGuiRender()
	{
